...
app.get('/flask-status', async (req, res) => {
    try {
        const response = await fetch(`http://localhost:${FLASK_PORT}/`, {
...
app.get('/progress', async (req, res) => {
    try {
        ...
        const response = await fetch(`http://localhost:${FLASK_PORT}/progress`, {
        ...
        if (error.code === 'ECONNREFUSED' || error.name === 'TypeError') {
            ...
            try {
                ...
                // Retry the request
                const retryResponse = await fetch(`http://localhost:${FLASK_PORT}/progress`);
...
app.post('/convert', async (req, res) => {
    try {
        ...
        const response = await fetch(`http://localhost:${FLASK_PORT}/convert`, {
            method: 'POST',
            body: req,
            headers: req.headers,
            duplex: 'half',
...
app.listen(PORT, () => {
    ...
    setTimeout(async () => {
        try {
            ...
            setTimeout(async () => {
                try {
                    const testResponse = await fetch(`http://localhost:${FLASK_PORT}/`, {
